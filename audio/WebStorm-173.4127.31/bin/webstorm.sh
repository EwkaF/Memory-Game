...
if [ -z "$JDK" -a -s "$HOME/.WebStorm2017.3/config/webstorm.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.WebStorm2017.3/config/webstorm.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.WebStorm2017.3/config/webstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.WebStorm2017.3/config/webstorm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$AGENT_PATH" ]; then
    AGENT="-agentpath:$AGENT_PATH=disablealloc,delay=10000,probe_disable=*,sessionname=WebStorm2017.3"
  fi
fi
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=WebStorm2017.3 \
